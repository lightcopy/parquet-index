...
| 2.0.2 | [0.2.2](http://spark-packages.org/package/lightcopy/parquet-index) |
| 2.1.x | Not supported |
...
```shell
 $SPARK_HOME/bin/spark-shell --packages lightcopy:parquet-index:0.2.2-s_2.11
```
Or for `pyspark` to use Python API (see section below):
```shell
$SPARK_HOME/bin/pyspark --packages lightcopy:parquet-index:0.2.2-s_2.11
```
Change to `lightcopy:parquet-index:0.2.2-s_2.10` for Scala 2.10.x
